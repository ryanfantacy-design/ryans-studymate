...
const ACCENTS = ["#FFD600", "#00C896", "#5BC0EB", "#B388FF", "#FF6B6B", "#FFB38A"];
const MOODS = [
...
const SubjectsView = ({ subjects, refresh }) => {
  ...
  const startEdit = (s) => { setEditingId(s.id); setEditName(s.name); setEditTarget(s.target_minutes_daily); };
  const saveEdit = async (id) => {
    await axios.put(`${API}/subjects/${id}`, { name: editName, target_minutes_daily: Number(editTarget) });
    setEditingId(null);
    refresh();
  };
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={add} data-testid="form-add-subject" className="nb-card p-6">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
          <input data-testid="subject-name-input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Subject name (e.g. Mathematics)" className="md:col-span-5 nb-input" />
          ...
          <div className="md:col-span-3 flex items-center gap-2 nb-input" style={{ padding: "8px 12px" }}>
            {ACCENTS.map((c) => (
              <button type="button" key={c} aria-label={`color-${c}`} onClick={() => setColor(c)}
                className={`h-6 w-6 rounded-full border-2 border-[#1a1a2e] transition-transform ${color === c ? "scale-125" : ""}`}
                style={{ background: c }} />
            ))}
          </div>
...
const PomodoroView = ({ subjects, settings, onLogged }) => {
  ...
  useEffect(() => { setRemaining(mode === "work" ? work : brk); }, [mode, work, brk]);
          ...
          if (r <= 1) {
            ...
            if (mode === "work" && subjectId) {
              axios.post(`${API}/sessions`, {
                subject_id: subjectId, duration_minutes: Math.round(work / 60),
                notes: "Pomodoro session", date: todayStr(),
  ...
  const ringColor = mode === "work" ? "#00C896" : "#5BC0EB";
  const RADIUS = 110;
    ...
    <div className="space-y-6">
      ...
      <div className="nb-card p-8 flex flex-col items-center" style={{ background: "#FFF" }}>
        <div className="flex gap-2 mb-6">
          <button data-testid="pomo-mode-work" onClick={() => { setRunning(false); setMode("work"); }}
            className={`nb-btn px-4 py-2 text-sm ${mode === "work" ? "bg-[#00C896]" : "bg-white"}`}>
            Work · {settings?.pomodoro_work || 25}m
          </button>
          <button data-testid="pomo-mode-break" onClick={() => { setRunning(false); setMode("break"); }}
            className={`nb-btn px-4 py-2 text-sm ${mode === "break" ? "bg-[#5BC0EB]" : "bg-white"}`}>
        ...
        <div className="relative" style={{ width: 260, height: 260 }}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <div className="text-xs uppercase tracking-widest text-[#4b4b63] mt-2 font-bold">{mode}</div>
          </div>
        </div>
        <div className="mt-6 flex items-center gap-3 flex-wrap justify-center">
          ...
          <button data-testid="pomo-toggle-btn" onClick={() => setRunning((r) => !r)} disabled={!subjectId && mode === "work"}
          ...
          <button data-testid="pomo-reset-btn" onClick={() => { setRunning(false); setRemaining(total); }} className="nb-btn bg-white px-4 py-3">
...
const ScheduleView = () => {
  ...
  const playAlarm = () => {
    if (audioRef.current) { try { audioRef.current.currentTime = 0; audioRef.current.play().catch(() => {}); } catch (e) {} }
    else if (music?.dataUrl) { const a = new Audio(music.dataUrl); a.play().catch(() => {}); }
    else {
      try {
        const ctx = new (window.AudioContext || window.webkitAudioContext)();
        for (let i = 0; i < 3; i++) {
  ...
  const onMusicUpload = (e) => {
    ...
    reader.onload = () => { const obj = { name: file.name, dataUrl: reader.result }; setMusic(obj); localStorage.setItem(MUSIC_KEY, JSON.stringify(obj)); };
    reader.readAsDataURL(file);
  };
  const clearMusic = () => { setMusic(null); localStorage.removeItem(MUSIC_KEY); };
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {music && (
            <>
              ...
              <button onClick={() => audioRef.current && audioRef.current.play()} data-testid="preview-music-btn" className="nb-btn bg-white px-3 py-1.5 text-xs">Preview</button>
